...
def register(request):
    if request.method == 'POST':
        ...
        profile, created = Profile.objects.get_or_create(
            user=user,  # Si ya existe un Profile, no lo creará de nuevo
            defaults={'photo': face_image}  # Solo se usa si se crea un nuevo perfil
        )
        
        if not created:
            # Si ya existe el perfil, actualiza la imagen
            profile.photo = face_image
            profile.save()
        ...
        student = Student.objects.create(
            ...
            username=user,
            email=email,
            image=face_image  # Usa la misma imagen que en Profile
        )
        student.save()
...
def cursos(request):
    categories = [
        {'name': 'matematicas', 'image': 'img/Servicios/matematicas.jpeg'},
        {'name': 'influencer', 'image': 'img/Servicios/influencer.png'},
        {'name': 'electronica', 'image': 'img/Servicios/electronica.png'},
        {'name': 'programacion', 'image': 'img/Servicios/programacion.jpg'},
    ]
...
def send_confirmation_email(student, course, enrollment):
    ...
    """
    subject = "Confirmación de Matriculacion de Curso"
    ...
    if request.method == 'POST':
        ...
        if 'expel_student' in request.POST:
            ...
            try:
                ...
                messages.success(request, 'Nota subida al portal y estudiante dado de baja por buen tipo')
...
def info_student(request):
    # Obtener el estudiante asociado al usuario actual
    student = Student.objects.get(username=request.user)
    return render(request, 'generales/info_student.html', {
        'student': student,
    })

@login_required
def info_teacher(request):
    # Obtener el estudiante asociado al usuario actual
    teacher = Teacher.objects.get(user=request.user)
